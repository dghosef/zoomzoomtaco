...
void CodeGen_C::visit(const Function* func) {
  ...
  } else if (false && func->name == "compute"){
    std::cout << "REMOVE THIS LATER. It only works for spmv" << std::endl;
    out << 
    R"V0G0N(
      // cksum should start with 694
void useless_assign() {
int a;
int b;
a = 10;
...
void nothing(){}
int compute(taco_tensor_t *A98, taco_tensor_t *pwtk, taco_tensor_t *A65) {
  int A981_dimension = (int)(A98->dimensions[0]);
  double* restrict A98_vals = (double*)(A98->vals);
  ...
  for (int32_t i101 = 0; i101 < pwtk1_dimension; i101++) {
    double ti102A98_val = 0.0;
    int32_t i102pwtk = pwtk2_pos[i101];
    int32_t *pwtk2_crd_p = pwtk2_crd + i102pwtk;
    double *pwtk_vals_p = pwtk_vals + i102pwtk;
    int endbound = pwtk2_pos[i101+1];
    #define make_reg(varname, type) register type varname##_reg = varname;
    #define restore(varname) varname = varname##_reg;
    make_reg(ti102A98_val, double);
    make_reg(pwtk_vals_p, double*);
    make_reg(pwtk2_crd_p, int*);
    make_reg(A65_vals, double*);
    for(int i = i102pwtk; i < endbound; i++) {
      ti102A98_val_reg += *pwtk_vals_p_reg * A65_vals[*pwtk2_crd_p];
       pwtk_vals_p_reg++;
      pwtk2_crd_p++;
    }
    restore(ti102A98_val);
    restore(pwtk_vals_p);
    restore(pwtk2_crd_p);
    restore(A65_vals);
    A98_vals[i101] = ti102A98_val;
  }
  return 0;
    // handwritten
}

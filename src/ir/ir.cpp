...
Stmt For::make(Expr var, Expr start, Expr end, Expr increment, Stmt body,
  ...
  // find all array indexes w/ loop var
  // Make a new pointer variable with the location of the first array index
  // Replace the array index with a dereference of that pointer
  // At the end increment the pointer

  // Cache the end bound into a variable
  // TODO: Move to simplify.cpp
  ...
  loop->var = var;
  loop->start = start;
  if(end.ptr->type_info() != IRNodeType::Var) {
  ...
  struct RewriteArrayAccesses : IRRewriter {
    Expr index_variable;
    Expr start;
    // of form <var, initial value>
    std::set<std::pair<Expr, Expr>> toDecl;
    RewriteArrayAccesses(Expr v, Expr s) {
      index_variable = v;
      start = s;
    ...
    void visit(const Load* op) {
      ...
      else if(isa<GetProperty>(loc)) {
        loc_name = to<GetProperty>(loc)->name;
      }
      else {
        expr = op;
        return;
      ...
      expr = op; return;
      if(isa<Var>(index_variable)) {
        if(to<Var>(index_variable)->name == loc_name) {
          auto ptr = Var::make(loc_name + "_p", op->type, true, false, false);
          expr = Load::make(ptr, 0);
          toDecl.insert({ptr, Add::make(op->arr, start)});
        }
        else {
          expr = op;
        }
  ...
  auto rewriter = RewriteArrayAccesses(var, start);
  loop->contents = rewriter.rewrite(loop->contents);
  for(auto todecl : rewriter.toDecl) {
    ret->append(VarDecl::make(todecl.first, todecl.second));
  }
  loop->kind = kind;

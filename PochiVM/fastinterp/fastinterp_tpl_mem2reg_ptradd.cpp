...
struct FIMem2RegPtrAdd
{
                  ...
                  OpaqueParams... opaqueParams) noexcept
    {
        ...
        ArrType result = arr + index;
        if constexpr(!spillOutput)
        {
            DEFINE_BOILERPLATE_FNPTR_PLACEHOLDER_0(void(*)(uintptr_t, GEN_MEM2REG_EXTRACT_TYPES(true, arrOrd, ArrType), OpaqueParams..., ArrType) noexcept);
            BOILERPLATE_FNPTR_PLACEHOLDER_0(stackframe, PASS_MEM2REG_PARAMS, opaqueParams..., result);
        }
        else
        {
            DEFINE_INDEX_CONSTANT_PLACEHOLDER_0;
            *GetLocalVarAddress<ArrType>(stackframe, CONSTANT_PLACEHOLDER_0) = result;
...
void __pochivm_build_fast_interp_library__()
{
    ...
    RegisterBoilerplate<FIMem2RegPtrAdd>();
}

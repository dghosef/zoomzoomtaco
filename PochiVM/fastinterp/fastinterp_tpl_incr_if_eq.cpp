...
//
struct FIIncrIfEqImpl
{
    template<typename OperandType>
    static constexpr bool cond()
    {
        // TODO: Better checking + remove the operand thing
    ...
    static void f(uintptr_t stackframe, OpaqueParams... opaqueParams) noexcept
    {
        ...
        // OperandType result = EvaluateArithmeticExpression<OperandType, operatorType>(lhs, rhs) + lhs + rhs;

        *GetLocalVarAddress<OperandType>(stackframe, CONSTANT_PLACEHOLDER_0) += static_cast<OperandType>(lhs == rhs);
...
void __pochivm_build_fast_interp_library__()
{
    ...
    RegisterBoilerplate<FIIncrIfEqImpl>();
}

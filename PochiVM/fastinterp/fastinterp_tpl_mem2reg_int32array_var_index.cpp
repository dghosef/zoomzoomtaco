...
struct FIMem2RegInt32ArrVarIdx
{
                  ...
                  OpaqueParams... opaqueParams) noexcept
    {
        ...
        int32_t *arr = reinterpret_cast<int32_t *>(EXTRACT_MEM2REG_VALUE(true, ord1));
        uint64_t idx = static_cast<uint64_t>(EXTRACT_MEM2REG_VALUE(true, ord2));
        int32_t result = arr[idx];
        if constexpr(!spillOutput)
        {
            DEFINE_BOILERPLATE_FNPTR_PLACEHOLDER_0(void(*)(uintptr_t, GEN_MEM2REG_EXTRACT_TYPES(true, ord1, uint64_t), OpaqueParams..., int32_t) noexcept);
            BOILERPLATE_FNPTR_PLACEHOLDER_0(stackframe, PASS_MEM2REG_PARAMS, opaqueParams..., result);
        }
        else
        {
            DEFINE_INDEX_CONSTANT_PLACEHOLDER_0;
            *GetLocalVarAddress<int32_t>(stackframe, CONSTANT_PLACEHOLDER_0) = result;
...
void __pochivm_build_fast_interp_library__()
{
    ...
    RegisterBoilerplate<FIMem2RegInt32ArrVarIdx>();
}

...
//
template<typename T>
Value<void> Assign(const Reference<T>& lhs, const Value<T>& rhs, bool isIncrement = false)
{
    auto assignExpr = new AstAssignExpr(lhs.__pochivm_ref_ptr, rhs.__pochivm_value_ptr);
    assignExpr->SetIncrement(isIncrement);
    return Value<void>(assignExpr);

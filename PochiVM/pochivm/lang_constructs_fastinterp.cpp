                                                                                    ...
                                                                                    FastInterpBoilerplateInstance* falseBr)
{
    ...
    if (cond->GetAstNodeType() == AstNodeType::AstComparisonExpr)
    {
        ...
        // Check for the form int32 < int32
        // where both ints are in registers
        if constexpr (isFavourTrueBranch) {
            if(expr->m_lhs->GetAstNodeType() == AstNodeType::AstRegisterCachedVariableExpr &&
               expr->m_rhs->GetAstNodeType() == AstNodeType::AstRegisterCachedVariableExpr) {
                   auto lhs = assert_cast<AstRegisterCachedVariableExpr*>(expr->m_lhs);
                   auto rhs = assert_cast<AstRegisterCachedVariableExpr*>(expr->m_rhs);
                   if(expr->m_op == AstComparisonExprType::LESS_THAN) {
                       if(cmpType.IsPrimitiveType() && cmpType.IsSigned() && cmpType.Size() == 4) {
                            FastInterpBoilerplateInstance* condBrInst = thread_pochiVMContext->m_fastInterpEngine->InstantiateBoilerplate(
                                        FastInterpBoilerplateLibrary<FIMem2RegInt32CompCondBranchImpl>::SelectBoilerplateBluePrint(

...
{
public:
    AstAssignExpr(AstNodeBase* dst, AstNodeBase* src, bool isIncrement=false)
        : AstNodeBase(AstNodeType::AstAssignExpr, TypeId::Get<void>())
        , m_isIncrement(isIncrement), m_fiInlineShape(FIShape::INVALID), m_isLhsMem2RegVar(false), m_dst(dst), m_src(src), m_mem2regDst(nullptr)
    {
    ...
    void SetIncrement(bool inc) {m_isIncrement = inc;}

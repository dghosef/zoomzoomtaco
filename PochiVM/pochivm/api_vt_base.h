...
inline Value<void> Assign(const ReferenceVT& lhs, const ValueVT& rhs, bool isIncrement=false)
{
  if (lhs.__pochivm_value_ptr->GetTypeId() ==
      rhs.__pochivm_value_ptr->GetTypeId()) {
    return Value<void>(
        new AstAssignExpr(lhs.__pochivm_ref_ptr, rhs.__pochivm_value_ptr, isIncrement));
  }
  // Implicit conversion
  if (rhs.__pochivm_value_ptr->GetTypeId().MayStaticCastTo(
    ...
    return Value<void>(new AstAssignExpr(lhs.__pochivm_ref_ptr, casted_rhs, isIncrement));
  }
